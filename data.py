...
def load_set(dataset, split="train", reduce_ratio=1.0, fixed_size=None):
    ...
    loaded_dataset = loaded_dataset.map(prepare_data, fn_kwargs={"reduce_ratio": reduce_ratio, "fixed_size": fixed_size})
...
class GrandStaffDataset(LightningDataModule):
    def __init__(self, config:ExperimentConfig) -> None:
        ...
        self.train_set = GrandStaffSingleSystem(data_path=self.data_path, split="train")
    ...
    def train_dataloader(self):
        return torch.utils.data.DataLoader(self.train_set, batch_size=self.batch_size, num_workers=self.num_workers, shuffle=False, collate_fn=batch_preparation_img2seq)
